#!/bin/bash
# diva — one-command hook installer for Claude Code
# Installs: SessionStart hook (auto-load rules) + UserPromptSubmit hook (mode tracking)
# Usage: bash hooks/install.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/JuliusBrussee/diva/main/hooks/install.sh)
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the diva hooks (used to merge"
...
REPO_URL="https://raw.githubusercontent.com/JuliusBrussee/diva/main/hooks"

HOOK_FILES=("package.json" "diva-config.js" "diva-activate.js" "diva-mode-tracker.js" "diva-statusline.sh")
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if DIVA_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.DIVA_SETTINGS, 'utf8'));
      const hasDivaHook = (event) =>
        Array.isArray(settings.hooks?.[event]) &&
        settings.hooks[event].some(e =>
          e.hooks && e.hooks.some(h => h.command && h.command.includes('diva'))
        );
      process.exit(
        hasDivaHook('SessionStart') &&
        hasDivaHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "Diva hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/diva-activate.js" ]; then
  echo "Reinstalling diva hooks (--force)..."
else
  echo "Installing diva hooks..."
fi
...
chmod +x "$HOOKS_DIR/diva-statusline.sh"
...
DIVA_SETTINGS="$SETTINGS" DIVA_HOOKS_DIR="$HOOKS_DIR" node -e "
  const fs = require('fs');
  const settingsPath = process.env.DIVA_SETTINGS;
  const hooksDir = process.env.DIVA_HOOKS_DIR;
  const managedStatusLinePath = hooksDir + '/diva-statusline.sh';
  const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
  if (!settings.hooks) settings.hooks = {};

  // SessionStart — auto-load diva rules
  ...
  if (!hasStart) {
    settings.hooks.SessionStart.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/diva-activate.js\"',
        timeout: 5,
        statusMessage: 'Loading diva mode...'
      }]
    });
  }

  // UserPromptSubmit — track mode changes when user types /diva commands
  ...
  if (!hasPrompt) {
    settings.hooks.UserPromptSubmit.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/diva-mode-tracker.js\"',
        timeout: 5,
        statusMessage: 'Tracking diva mode...'
      }]
    });
  }

  // Statusline — wire diva badge (report if skipped)
  if (!settings.statusLine) {
    ...
    if (cmd.includes(managedStatusLinePath)) {
      ...
      console.log('  NOTE: Existing statusline detected — diva badge NOT added.');
...
echo "  - SessionStart hook: auto-loads diva rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/diva lite, /diva ultra, /diva-commit, etc.)"
echo "  - Statusline badge: shows [DIVA] or [DIVA:ULTRA] etc."
