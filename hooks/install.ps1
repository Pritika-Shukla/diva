# diva — one-command hook installer for Claude Code (Windows PowerShell)
...
#        irm https://raw.githubusercontent.com/JuliusBrussee/diva/main/hooks/install.ps1 | iex
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: 'node' is required to install the diva hooks (used to merge" -ForegroundColor Red
...
$RepoUrl = "https://raw.githubusercontent.com/JuliusBrussee/diva/main/hooks"

$HookFiles = @("package.json", "diva-config.js", "diva-activate.js", "diva-mode-tracker.js", "diva-statusline.sh", "diva-statusline.ps1")
...
if (-not $Force) {
    ...
    if ($AllFilesPresent -and (Test-Path $Settings)) {
        try {
                ...
                foreach ($entry in $entries) {
                    if ($entry.hooks) {
                        foreach ($hookDef in $entry.hooks) {
                            if ($hookDef.command -and $hookDef.command.Contains("diva")) {
                                return $true
                            }
                        }
                    }
            ...
            $HooksWired = (& $hasDivaHook "SessionStart") -and (& $hasDivaHook "UserPromptSubmit")
    ...
    if ($AllFilesPresent -and $HooksWired -and $HasStatusLine) {
        Write-Host "Diva hooks already installed in $HooksDir"
...
if ($Force -and (Test-Path (Join-Path $HooksDir "diva-activate.js"))) {
    Write-Host "Reinstalling diva hooks (-Force)..."
} else {
    Write-Host "Installing diva hooks..."
}
...
$env:DIVA_SETTINGS = $Settings -replace '\\', '/'
$env:DIVA_HOOKS_DIR = $HooksDir -replace '\\', '/'

$nodeScript = @'
const fs = require('fs');
const settingsPath = process.env.DIVA_SETTINGS;
const hooksDir = process.env.DIVA_HOOKS_DIR;
const managedStatusLinePath = hooksDir + '/diva-statusline.ps1';
...
Write-Host "  - SessionStart hook: auto-loads diva rules every session"
Write-Host "  - Mode tracker hook: updates statusline badge when you switch modes"
Write-Host "    (/diva lite, /diva ultra, /diva-commit, etc.)"
Write-Host "  - Statusline badge: shows [DIVA] or [DIVA:ULTRA] etc."
