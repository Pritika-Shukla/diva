#!/usr/bin/env node
// diva — shared configuration resolver
//
// Resolution order for default mode:
//   1. DIVA_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/diva/config.json (any platform, if set)
//      - ~/.config/diva/config.json (macOS / Linux fallback)
//      - %APPDATA%\diva\config.json (Windows fallback)
//   3. 'full'
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'diva');
  }
  ...
  return path.join(os.homedir(), '.config', 'diva');
}
...
function getDefaultMode() {
  ...
  const envMode = process.env.DIVA_DEFAULT_MODE;
...
// flag path (~/.claude/.diva-active) with a symlink to clobber other files.
//
...
function safeWriteFlag(flagPath, content) {
  try {
    ...
    const tempPath = path.join(flagDir, `.diva-active.${process.pid}.${Date.now()}`);
