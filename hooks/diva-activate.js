#!/usr/bin/env node
// diva — Claude Code SessionStart activation hook
//
// Runs on every session start:
//   1. Writes flag file at $CLAUDE_CONFIG_DIR/.diva-active (statusline reads this)
//   2. Emits diva ruleset as hidden SessionStart context
//   3. Detects missing statusline config and emits setup nudge

const fs = require('fs');
const path = require('path');
const os = require('os');
const { getDefaultMode, safeWriteFlag } = require('./diva-config');
...
// 2. Emit full diva ruleset, filtered to the active intensity level.
...
// Modes that have their own independent skill files — not diva intensity levels.
// For these, emit a short activation line; the skill itself handles behavior.
const INDEPENDENT_MODES = new Set(['commit', 'review', 'compress']);

if (INDEPENDENT_MODES.has(mode)) {
  process.stdout.write('DIVA MODE ACTIVE — level: ' + mode + '. Behavior defined by /diva-' + mode + ' skill.');
  process.exit(0);
}
...
// Read SKILL.md — the single source of truth for diva behavior.
// Plugin installs: __dirname = <plugin_root>/hooks/, SKILL.md at <plugin_root>/skills/diva/SKILL.md
// Standalone installs: __dirname = $CLAUDE_CONFIG_DIR/hooks/, SKILL.md won't exist — falls back to hardcoded rules.
let skillContent = '';
try {
  skillContent = fs.readFileSync(
    path.join(__dirname, '..', 'skills', 'diva', 'SKILL.md'), 'utf8'
  );
...
if (skillContent) {
  ...
  output = 'DIVA MODE ACTIVE — level: ' + modeLabel + '\n\n' + filtered.join('\n');
} else {
  // Fallback when SKILL.md is not found (standalone hook install without skills dir).
  // This is the minimum viable ruleset — better than nothing.
  output =
    'DIVA MODE ACTIVE — level: ' + modeLabel + '\n\n' +
    'Respond terse like smart diva. All technical substance stay. Only fluff die.\n\n' +
    '## Persistence\n\n' +
    'ACTIVE EVERY RESPONSE. No revert after many turns. No filler drift. Still active if unsure. Off only: "stop diva" / "normal mode".\n\n' +
    'Current level: **' + modeLabel + '**. Switch: `/diva lite|full|ultra`.\n\n' +
    '## Rules\n\n' +
    ...
    '## Boundaries\n\n' +
    'Code/commits/PRs: write normal. "stop diva" or "normal mode": revert. Level persist until changed or session end.';
}
...
try {
  ...
  if (!hasStatusline) {
    ...
    const scriptName = isWindows ? 'diva-statusline.ps1' : 'diva-statusline.sh';
    ...
    output += "\n\n" +
      "STATUSLINE SETUP NEEDED: The diva plugin includes a statusline badge showing active mode " +
      "(e.g. [DIVA], [DIVA:ULTRA]). It is not configured yet. " +
